 ...
 * in it with no content record reads as an orphan). The hover tint below settles it outright: a
 * file cannot be recoloured on hover at all.
 *
 ...
 *
 * **The brand colour lives here beside the path because it is the same kind of fact**: what the
 * mark is, and what colour that mark is when it is being itself. It reaches the stylesheet as a
 * pair of custom properties rather than as a finished `light-dark()`, because inline styles are
 * not processed by Lightning CSS and so would miss the polyfill every other themed value in this
 * codebase goes through — `.contactCompact` composes the pair with `light-dark()` in CSS, where
 * it does. Two of the five are near-black wordmarks with no colour to go to, so their dark entry
 * is white: on the dark ground the brand *is* the inversion.
 */
type PlatformMark = {
  d: string;
  /** The export's own fill rule. GitHub's octocat and Unsplash's notch both need it. */
  evenOdd?: boolean;
  /** `[light, dark]`, composed into one `light-dark()` by `.contactCompact`. */
  brand: [string, string];
};
...
/**
 * That platform's brand colour, as the light/dark pair `.contactCompact` composes. Returned as
 * a style object so the caller cannot spread only half of it.
 */
export function platformBrandStyle(platform: string): React.CSSProperties | undefined {
  const mark = PLATFORM_MARKS[platform];
  if (!mark) return undefined;
  return {
    '--contact-brand-light': mark.brand[0],
    '--contact-brand-dark': mark.brand[1],
